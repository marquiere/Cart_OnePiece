...
# -----------------
x2 = x1 + bw + space_x
draw_box(ax, x2, row1_y, bw, bh, "2. Data Ingestion &\nSynchronization", 
         "• C++ Async Listeners\n• Buffer Ring Allocation\n• FrameSync Threading\n• Dispatch to Engine", 
         c_cpp['ec'], c_cpp['fc'], c_cpp['tfc'])
draw_arrow(ax, x1+bw, row1_y+bh/2, x2, row1_y+bh/2)
ax.text(x1+bw+space_x/2, row1_y+bh/2+1, "Raw BGRA\n& Ground Truth", ha='center', va='bottom', fontsize=9, color='#4A5568')
...
# -----------------
x3 = sp_con_x + 4
draw_box(ax, x3, row1_y, bw, bh, "3. Task: Preprocessing", 
         "• CPU Worker Execution\n• Crop & Resize (Bilinear)\n• Channel Swapping\n• Mean/Std Normalization\n• Float32 Tensor Format", 
...
# -----------------
x5 = x4 + bw + space_x
draw_box(ax, x5, row1_y, bw, bh, "5. Task: Post-processing", 
         "• CPU Worker Execution\n• Logit Argmax Decoding\n• Direct Label Unpacking\n• Format for Evaluation", 
         c_starpu['ec'], 'white', c_starpu['tfc'])
draw_arrow(ax, x4+bw, row1_y+bh/2, x5, row1_y+bh/2)
ax.text(x4+bw+space_x/2, row1_y+bh/2+1, "Raw Logits\nDependence", ha='center', va='bottom', fontsize=9, color='#4A5568', fontweight='bold')
...
ew = bw * 1.8
draw_box(ax, x6, y6, ew, bh, "6. Offline Metric Evaluation", 
         "• Asynchronous C++ Thread (Post-callback)\n• Ground Truth Comparison (mIoU / Pixel Accuracy)\n• Latency measurements via std::chrono", 
...
filename = '/home/eric/Cart_OnePiece/Cart-OnePiece_Antigravity_C++/Tese_Ericson/imagens/pipeline_architecture_v2.png'
