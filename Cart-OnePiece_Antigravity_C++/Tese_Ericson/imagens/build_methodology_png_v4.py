...
colors = [
    {'border': '#3381B1', 'top': '#D1E6F5', 'bot': '#EDF5FA', 'stage': 'Stage 1:', 'title': 'Systematic\nMapping Study', 
     'bullets': ['- Collect and\n  analyze existing\n  literature', '- Identify key\n  research trends\n  and gaps', '- Classify prior\n  work']},
    {'border': '#49A254', 'top': '#CDF2D3', 'bot': '#EDF9F0', 'stage': 'Stage 2:', 'title': 'Experimental\nFeasibility',
     'bullets': ['- Develop initial\n  prototypes', '- Conduct\n  proof-of-concept\n  experiments', '- Assess\n  technical\n  viability']},
    {'border': '#DE9135', 'top': '#F9DFB8', 'bot': '#FEF4E8', 'stage': 'Stage 3:', 'title': 'Cart-OnePiece\nFramework',
     'bullets': ['- Propose new\n  theoretical\n  model', '- Implement the\n  Cart-OnePiece\n  system', '- Integrate\n  components\n  and protocols']},
    {'border': '#C53641', 'top': '#F9C8C9', 'bot': '#FDECEC', 'stage': 'Stage 4:', 'title': 'Empirical\nEvaluation',
...
for i, c in enumerate(colors):
    ...
    top_rect = patches.Rectangle((bx-4, split_y), box_w+8, by+box_h-split_y+4, facecolor=c['top'], alpha=0.6, zorder=1) # slightly translucent top
    ...
    if i == 0:
        ...
        ax.add_patch(patches.Circle((ch_cx, ch_cy), 3.5, edgecolor=c['border'], facecolor='white', lw=1.8, zorder=4))
        ax.plot([ch_cx-1.2, ch_cx-0.2, ch_cx+1.8], [ch_cy-0.4, ch_cy-1.2, ch_cy+1.2], color=c['border'], lw=1.8, zorder=5)
    ...
    for btext in c['bullets']:
        lines = btext.split('\n')
        ax.text(text_x, cur_y, lines[0], color='#000000', fontfamily='sans-serif', 
                fontsize=15, ha='left', va='top', zorder=4)
        cur_y -= 8.5
        for l in lines[1:]:
            ax.text(text_x, cur_y, l, color='#000000', fontfamily='sans-serif', 
                    fontsize=15, ha='left', va='top', zorder=4)
            cur_y -= 8.5
        cur_y -= 2.5 

plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
filename = '/home/eric/Cart_OnePiece/Cart-OnePiece_Antigravity_C++/Tese_Ericson/imagens/methodology_stages_v4_twotone_saturated.png'
