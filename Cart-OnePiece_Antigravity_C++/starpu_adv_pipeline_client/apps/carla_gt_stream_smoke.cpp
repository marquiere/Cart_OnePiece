...
int main(int argc, char **argv) {
  ...
  std::cout << "Connecting to CARLA at " << host << ":" << port << "..."
            << std::endl;
  std::cout << "Assume BGRA layout: " << (assume_bgra ? "YES" : "NO")
            << std::endl;
  ...
  try {
    ...
    std::cout << "Spawned GT Camera ID: " << cam_actor->GetId() << std::endl;
    ...
    camera->Listen([&](auto data) {
      ...
      // Decode and Stats (Done on every print_every interval for feedback)
      if (frame_count % print_every == 0 || frame_count == frames_to_capture) {
        // A) frame_id, timestamp
        std::cout << "\n--- [GT Stream] Frame " << frame_id
                  << " | Time: " << timestamp << " | Captured: " << frame_count
                  << "/" << frames_to_capture
                  << " | Drops Approx: " << drop_estimate << " ---"
                  << std::endl;

        // B) Decode
        auto labels = DecodeSemanticLabels(
            reinterpret_cast<const uint8_t *>(img->data()), w, h, assume_bgra);
        auto stats = ComputeSemanticStats(labels);

        // C) Number of unique labels & list of up to 20
        std::cout << "Unique Labels (" << stats.unique_labels.size() << "): ";
        for (size_t i = 0; i < std::min<size_t>(stats.unique_labels.size(), 20);
             ++i) {
          std::cout << static_cast<int>(stats.unique_labels[i]) << " ";
        }
        if (stats.unique_labels.size() > 20)
          std::cout << "...";
        std::cout << std::endl;

        // D) Top 10 labels by pixel count
        std::cout << "Top Labels by Pixel Count:" << std::endl;
        size_t total_pixels = labels.size();
        for (const auto &pair : stats.top_k) {
          double pct =
              (static_cast<double>(pair.second) / total_pixels) * 100.0;
          std::cout << "  - Label " << std::setw(3)
                    << static_cast<int>(pair.first) << ": " << std::setw(7)
                    << pair.second << " px (" << std::fixed
  ...
  if (vehicle)
    vehicle->Destroy();
  std::cout << "Clean exit. GT Semantic Smoke test complete." << std::endl;
