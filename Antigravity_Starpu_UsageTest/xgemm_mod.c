...
static void init_problem_data(void) {
  ...
  starpu_malloc_flags((void **)&A, zdim * ydim * sizeof(TYPE),
                      STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
  starpu_malloc_flags((void **)&B, xdim * zdim * sizeof(TYPE),
                      STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
  starpu_malloc_flags((void **)&C, xdim * ydim * sizeof(TYPE),
                      STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
...
static void partition_mult_data(void) {
  ...
  starpu_matrix_data_register(&A_handle, STARPU_MAIN_RAM, (uintptr_t)A, ydim,
                              ydim, zdim, sizeof(TYPE));
  starpu_data_set_name(A_handle, "A");
  starpu_matrix_data_register(&B_handle, STARPU_MAIN_RAM, (uintptr_t)B, zdim,
                              zdim, xdim, sizeof(TYPE));
  starpu_data_set_name(B_handle, "B");
  starpu_matrix_data_register(&C_handle, STARPU_MAIN_RAM, (uintptr_t)C, ydim,
                              ydim, xdim, sizeof(TYPE));
...
static void cublas_mult(void *descr[], void *arg, const TYPE *beta) {
  ...
  cublasStatus_t status = CUBLAS_GEMM(
      starpu_cublas_get_local_handle(), CUBLAS_OP_N, CUBLAS_OP_N, nxC, nyC, nyA,
      &p1_cuda, subA, ldA, subB, ldB, beta, subC, ldC);
...
void cpu_mult(void *descr[], void *arg, TYPE beta) {
  ...
  if (worker_size == 1) {
    /* Sequential CPU task */
    CPU_GEMM("N", "N", nxC, nyC, nyA, (TYPE)1.0, subA, ldA, subB, ldB, beta,
             subC, ldC);
  } else {
    /* Parallel CPU task */
    ...
    unsigned new_nyC =
        STARPU_MIN(nyC, block_size * (rank + 1)) - block_size * rank;
    ...
    CPU_GEMM("N", "N", nxC, new_nyC, nyA, (TYPE)1.0, subA, ldA, new_subB, ldB,
             beta, new_subC, ldC);
  }
}
#endif
...
static void parse_args(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-3d") == 0) {
      ...
      fprintf(
          stderr,
          "Usage: %s [-3d] [-nblocks n] [-nblocksx x] [-nblocksy y] [-nblocksz "
          "z] [-x x] [-y y] [-xy n] [-z z] [-xyz n] [-size size] [-iter iter] "
          "[-bound] [-check] [-spmd] [-hostname] [-nsleeps nsleeps]\n",
          argv[0]);
      if (tiled)
        fprintf(stderr,
                "Currently selected: %zux%zu * %zux%zu and %zux%zux%zu blocks "
                "(size %zux%zu length %zu), %u iterations, %u sleeps\n",
                zdim, ydim, xdim, zdim, nslicesx, nslicesy, nslicesz,
                xdim / nslicesx, ydim / nslicesy, zdim / nslicesz, niter,
                nsleeps);
      else
        fprintf(stderr,
                "Currently selected: %zux%zu * %zux%zu and %zux%zu blocks "
                "(size %zux%zu length %zu), %u iterations, %u sleeps\n",
                zdim, ydim, xdim, zdim, nslicesx, nslicesy, xdim / nslicesx,
                ydim / nslicesy, zdim, niter, nsleeps);
      exit(EXIT_SUCCESS);
    } else {
...
static int run_data(void) {
  ...
  for (sleeps = 0; sleeps < nsleeps; sleeps++) {
      ...
      if (tiled) {
        for (x = 0; x < nslicesx; x++)
          for (y = 0; y < nslicesy; y++) {
            starpu_data_handle_t Ctile =
                starpu_data_get_sub_data(C_handle, 2, x, y);
            for (z = 0; z < nslicesz; z++) {
              struct starpu_codelet *cl = z == 0 ? &cl_gemm0 : &cl_gemm;
              int ret = starpu_task_insert(
                  cl, cl->modes[0], starpu_data_get_sub_data(A_handle, 2, z, y),
                  cl->modes[1], starpu_data_get_sub_data(B_handle, 2, x, z),
                  cl->modes[2], Ctile, STARPU_FLOPS,
                  (double)(2ULL * (xdim / nslicesx) * (ydim / nslicesy) *
                           (zdim / nslicesz)),
                  0);
              if (ret == -ENODEV) {
                check = 0;
        ...
        for (x = 0; x < nslicesx; x++)
          for (y = 0; y < nslicesy; y++) {
            int ret = starpu_task_insert(
                &cl_gemm0, cl_gemm0.modes[0],
                starpu_data_get_sub_data(A_handle, 1, y), cl_gemm0.modes[1],
                starpu_data_get_sub_data(B_handle, 1, x), cl_gemm0.modes[2],
                starpu_data_get_sub_data(C_handle, 2, x, y), STARPU_FLOPS,
                (double)(2ULL * (xdim / nslicesx) * (ydim / nslicesy) * zdim),
                0);
            if (ret == -ENODEV) {
              check = 0;
              return 77;
    ...
    double min, min_int;
    double flops = 2.0 * ((unsigned long long)(niter)) *
                   ((unsigned long long)xdim) * ((unsigned long long)ydim) *
                   ((unsigned long long)zdim);
    ...
    PRINTF("%zu\t%zu\t%zu\t%.0f\t%.1f", xdim, ydim, zdim,
           timing / (niter) / 1000.0, flops / timing / 1000.0);
    if (bound)
      PRINTF("\t%.0f\t%.1f\t%.0f\t%.1f", min, flops / min / 1000000.0, min_int,
             flops / min_int / 1000000.0);
    PRINTF("\n");
