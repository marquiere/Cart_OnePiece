...
BUILD_DIR_DEFAULT="./tools/starpu_adv_pipeline/build"
...
if [ -f "$BUILD_DIR/starpu_adv_pipeline" ]; then
    BIN_PATH="$(readlink -f $BUILD_DIR/starpu_adv_pipeline)"
elif [ -f "./build/starpu_adv_pipeline" ]; then
    BIN_PATH="$(readlink -f ./build/starpu_adv_pipeline)"
else
    echo "ERROR: Could not locate starpu_adv_pipeline binary."
    exit 1
fi

# Construct Arguments
ARGS="--frames $FRAMES --mb $MB --infer-iters $INFER_ITERS --trace --trace-dir $DIR_RAW"
if [ -n "$SCHED" ]; then
    ARGS="$ARGS --sched $SCHED"
fi

if [ "$MEMORY_MODE" -eq 1 ]; then
    ARGS="$ARGS --print-stats"
fi

if [ "$TRANSFER_MB" -gt 0 ]; then
    ARGS="$ARGS --transfer-mb $TRANSFER_MB"
fi
...
if [ -s "distrib.data" ]; then
    if command -v starpu_codelet_profile &> /dev/null; then
        for symbol in adv_prep_cpu adv_infer_cuda adv_post_cpu adv_logger_cpu; do
...
if command -v starpu_fxt_data_trace &> /dev/null; then
    starpu_fxt_data_trace "$DIR_RAW"/prof_file_* adv_prep_cpu adv_infer_cuda adv_post_cpu adv_logger_cpu > data_trace.txt 2>&1 || true
fi
