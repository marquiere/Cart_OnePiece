...
static void init_problem_data(void) {
  ...
  starpu_malloc_flags((void **)&A, zdim * ydim * sizeof(float),
                      STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
  starpu_malloc_flags((void **)&B, xdim * zdim * sizeof(float),
                      STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
  starpu_malloc_flags((void **)&C, xdim * ydim * sizeof(float),
                      STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
...
static void partition_mult_data(void) {
  ...
  struct starpu_data_filter vert = {.filter_func =
                                        starpu_matrix_filter_vertical_block,
                                    .nchildren = nslicesx};

  struct starpu_data_filter horiz = {.filter_func = starpu_matrix_filter_block,
                                     .nchildren = nslicesy};

  /*
   *	Illustration with nslicex = 4 and nslicey = 2, it is possible to access
   *	sub-data by using the "starpu_data_get_sub_data" method, which takes a
   * data handle, the number of filters to apply, and the indexes for each
   ...
   *
   *	"starpu_data_get_sub_data(C_handle, 1, 3)" would return a handle to the
   * 4th column of blocked matrix C for example.
   *
   *		              |---|---|---|---|
   *		              |   |   | B'|   | B
   *		              |---|---|---|---|
...
static struct starpu_perfmodel mult_perf_model = {.type = STARPU_HISTORY_BASED,
...
static int launch_tasks(void) {
  ...
  for (taskx = 0; taskx < nslicesx; taskx++) {
    for (tasky = 0; tasky < nslicesy; tasky++) {
       ...
       * equivalent to
       * starpu_data_get_sub_data(starpu_data_get_sub_data(C_handle, 1, taskx),
       * 1, tasky)*/
  ...
  for (i = 0; i < ydim; i++) {
    for (j = 0; j < xdim; j++) {
      if (C_gpu[j + i * ldC] - C_ref[j + i * ldC] > 1e-6 * C_ref[j + i * ldC]) {
        printf("| Cref[%zu,%zu]=%f - Cgpu[%zu,%zu]=%f | Error in the "
               "computation of C: the difference between the two is bigger "
               "than 1e-6 * the reference",
...
int main(void) {
  ...
  starpu_free_flags(A, zdim * ydim * sizeof(float),
                    STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
  starpu_free_flags(B, xdim * zdim * sizeof(float),
                    STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
  starpu_free_flags(C, xdim * ydim * sizeof(float),
                    STARPU_MALLOC_PINNED | STARPU_MALLOC_SIMULATION_FOLDED);
